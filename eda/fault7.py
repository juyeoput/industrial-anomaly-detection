...
fault7 = faulty[faulty['faultNumber'] == 7]

run1_normal = normal[normal['simulationRun'] == 1].reset_index(drop=True)
run1_fault7 = fault7[fault7['simulationRun'] == 1].reset_index(drop=True)

fig, axes = plt.subplots(4, 1, figsize=(14, 12))

sensors = ['xmeas_4', 'xmeas_7', 'xmeas_9', 'xmeas_6']
titles  = [
    'A/C Feed Flow (kscmh)',
...
for i, (sensor, title) in enumerate(zip(sensors, titles)):
    ...
    axes[i].plot(run1_fault7[sensor], label='Fault 7', color='red', linewidth=0.8)
...
plt.tight_layout()
plt.savefig('fault7_comparison.png', dpi=150)
plt.show()
print("Saved: fault7_comparison.png")
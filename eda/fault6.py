...
fault6 = faulty[faulty['faultNumber'] == 6]

run1_normal = normal[normal['simulationRun'] == 1].reset_index(drop=True)
run1_fault6 = fault6[fault6['simulationRun'] == 1].reset_index(drop=True)
...
for i, (sensor, title) in enumerate(zip(sensors, titles)):
    ...
    axes[i].plot(run1_fault6[sensor], label='Fault 6', color='red', linewidth=0.8)
...
plt.tight_layout()
plt.savefig('fault6_comparison.png', dpi=150)
plt.show()
print("Saved: fault6_comparison.png")
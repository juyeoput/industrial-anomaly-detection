...
fault1 = faulty[faulty['faultNumber'] == 1]

run1_normal = normal[normal['simulationRun'] == 1].reset_index(drop=True)
run1_fault1 = fault1[fault1['simulationRun'] == 1].reset_index(drop=True)
...
for i, (sensor, title) in enumerate(zip(sensors, titles)):
    ...
    axes[i].plot(run1_fault1[sensor], label='Fault 1', color='red', linewidth=0.8)
...
plt.tight_layout()
plt.savefig('fault1_comparison.png', dpi=150)
plt.show()
print("Saved: fault1_comparison.png")
...
fault13 = faulty[faulty['faultNumber'] == 13]

run1_normal = normal[normal['simulationRun'] == 1].reset_index(drop=True)
run1_fault13 = fault13[fault13['simulationRun'] == 1].reset_index(drop=True)

fig, axes = plt.subplots(4, 1, figsize=(14, 12))

sensors = ['xmeas_9', 'xmeas_7', 'xmeas_6', 'xmeas_17']
titles  = [
...
for i, (sensor, title) in enumerate(zip(sensors, titles)):
    ...
    axes[i].plot(run1_fault13[sensor], label='Fault 13', color='red', linewidth=0.8)
...
plt.tight_layout()
plt.savefig('fault13_comparison.png', dpi=150)
plt.show()
print("Saved: fault13_comparison.png")
...
fault4 = faulty[faulty['faultNumber'] == 4]

run1_normal = normal[normal['simulationRun'] == 1].reset_index(drop=True)
run1_fault4 = fault4[fault4['simulationRun'] == 1].reset_index(drop=True)
...
for i, (sensor, title) in enumerate(zip(sensors, titles)):
    ...
    axes[i].plot(run1_fault4[sensor], label='Fault 4', color='red', linewidth=0.8)
...
plt.tight_layout()
plt.savefig('fault4_comparison.png', dpi=150)
plt.show()
print("Saved: fault4_comparison.png")
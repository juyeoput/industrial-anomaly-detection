...
fault8 = faulty[faulty['faultNumber'] == 8]

run1_normal = normal[normal['simulationRun'] == 1].reset_index(drop=True)
run1_fault8 = fault8[fault8['simulationRun'] == 1].reset_index(drop=True)
...
for i, (sensor, title) in enumerate(zip(sensors, titles)):
    ...
    axes[i].plot(run1_fault8[sensor], label='Fault 8', color='red', linewidth=0.8)
...
plt.tight_layout()
plt.savefig('fault8_comparison.png', dpi=150)
plt.show()
print("Saved: fault8_comparison.png")
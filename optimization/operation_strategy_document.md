...
- [ ] Quantify each fault's OEE impact with actual TEP data (slow-down %, expected production loss)
- [ ] Bottleneck analysis: on fault, which of the 5 units accumulates downstream queue
- [ ] Convert this table into a format for the LLM prompt (e.g. JSON)
- [x] Re-check severity/persistence against v4 — done, all 10 severities unchanged

---

## 7. Source of Severity and Persistence

The Severity and Persistence values in this document are taken directly from
fault_scenario_document_v4.md, Section 4 (Fault Scenario Mapping Table)
and the v3 Detailed Fault Descriptions.

v4 update: The multi-run statistical validation (10 runs + Welch's t-test)
did NOT change any of the 10 severity levels, so all operational responses in
this document remain valid. v4 additionally confirmed Fault 7 as transient
(mean returns to normal by end of window), which reinforces its R2 (monitor-only)
classification here. v4's mean-shift vs variance-detection split is a modeling
concern (for the ML side), not an operational one, so it does not alter this table.